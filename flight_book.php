<?php
      ...
      $s=explode("&",$t1);
      $t12=$_SESSION["pnr"];
      $_SESSION["airline"]=substr($s[0],0,2);
      $_SESSION["flight"]=$s[0];
      $_SESSION["departure_time"]=$s[1];
      $_SESSION["arrival_time"]=$s[3];
      $_SESSION["price"]=$s[2];
      $sql = "UPDATE travel_details set `dep_flight`='$s[0]',`departure_time`='$s[1]',`arrival_time`='$s[3]',`aircraft`='$s[4]',departure='$s[5]',arrival='$s[6]' where `pnr`='$t12'";
      ...
      if ($conn->query($sql) === TRUE) 
      {
         ...
         if($s[7]!="")
         {
         header('location:flight_rbook.php'); 
         }
         else
         {
          if($_SESSION["fr"]=="")
...
<ul class="w3-ul w3-card-4">
  ...
  $d=strtoupper($_SESSION["dt"]);
  $a=strtoupper($_SESSION["ar"]);
  $td=strtoupper($_SESSION["td"]);
  $rd=strtoupper($_SESSION["rd"]);
  $sr=$st."&origin=".$d."&destination=".$a."&departure_date=".$td."&nonstop=true"."&currency=INR"."&number_of_results=10";
  ...
  for($x=0;$x<count($result);$x++)
  {
   ...
   <div class='w3-bar-item'>
   <span class='w3-large'>&emsp;&emsp;&emsp;$d&emsp;&emsp;&emsp;&emsp;&emsp;$a&emsp;&emsp;&emsp;&emsp;&#8377;$r[total_price]&emsp;&emsp;";
...
if($rd=="")
{
   ...
   echo "   <span class='w3-bar-item w3-button w3-small w3-right'>
            <button class='w3-deep-orange' type='submit' name='submit' value=$bv onClick='flight_book.php'>choose&nbsp;&#9992;</button></span></li><br>";
   echo "</form>";
}
else
  {
